...
public class ReceptionHistoryParse {
    public static void receptionHistory(Result result, ResultListener resultListener){
        if(result.isSuccess()){
    ...
    private static List<ReceHistoryData> parseOneMessage(String msg){
        List<ReceHistoryData> mList = new ArrayList<>();
        try{
            Type type = new TypeToken<List<String[]>>(){}.getType();
            Gson gson = new Gson();
            List<String[]> strsList = gson.fromJson(msg,type);
            for(String[] strs:strsList){
                mList.add(new ReceHistoryData(strs[0],strs[1],strs[2],strs[3],strs[4],strs[5],strs[6]));
            }
        }catch (Exception e){
